...
def ExtractOpCode(Instruction, LineNumber, Memory):
    if len(Instruction) > 9:
        OpCodeValues = ["LDA", "STA", "LDA#", "HLT", "ADD", "JMP", "SUB", "CMP#", "BEQ", "SKP", "JSR", "RTN", "ADD#", "SUB#", "CMP", "   "]
...
def ReportStackOverflowError(ErrorMessage, Registers):
    print("Run time error:", ErrorMessage)
    Registers[SO] = 1
    return Registers
...
def ExecuteADDimm(Registers, Operand):
    ...
    Registers = SetFlags(Registers[ACC], Registers)
    if Registers[STATUS] == ConvertToDecimal("001"):                            #sdfnbuidsfbysdiufbdsuyfvbs
...
def ExecuteSUBimm(Registers, Operand):
    Registers[ACC] = Registers[ACC] - Operand
    Registers = SetFlags(Registers[ACC], Registers)                            #sdfbdsuyfbdsuyfbdysfvdsfy
    if Registers[STATUS] == ConvertToDecimal("001"):
        ReportRunTimeError("Overflow", Registers)
    return Registers
...
def ExecuteCMP(Memory, Registers, Address):
    Value = Registers[ACC] - Memory[Address].OperandValue                            #dsfbhdsufydsbfuidbsfid
...
def ExecuteJSR(Memory, Registers, Address):
    ...
    if Memory[StackPointer].OperandValue != 0:
        ReportStackOverflowError("Stack Overflow", Registers)
